...
var _ = require("lodash"); // console.log("DbMysql", DbMysql);
...
module.exports = class DbTableExec {
  ...
  _createWhere(fromUpdate) {
    ...
    if (!this.where) {
      ...
      where += "1";
      Object.entries(this.where).forEach(function (_ref, index) {
        var [key, val] = _ref;
        where += " && " + key + "=?";
  ...
  _preTreatment() {
    ...
    for (var [fieldName, field] of Object.entries(this.def.attributes)) {
      ...
      if (field.type == "datetime" && this.data[fieldName]) {
        this.data[fieldName] = moment(new Date(this.data[fieldName])).format("YYYY-MM-DD HH:mm:ss");
      }

      if (field.type == "date" && this.data[fieldName]) {
        var m = moment(new Date(this.data[fieldName]));
      ...
      if (field.type == "datetime" && this.data[fieldName]) {
        var _m = moment(new Date(this.data[fieldName]));
  ...
  exec(cb, returnCompleteRow) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var rows = yield _this2.connection.query(query, _this2.whereData);
