    ...
    this._routes.forEach(route => {
      _App.App[route.method](route.url, (req, res, next) => {
        ...
        req.on("end", function () {
          ...
          console.warn("".concat((0, _dayjs.default)().format("YYYY-MM-DD HH:mm:ss"), " - ").concat(route.method.toUpperCase(), " ").concat(route.url, " - ").concat(diff));
        });
        next();
      });
    ...
    return _asyncToGenerator(function* () {
      ...
      function findCreateWhereForField(tx, field, value, defField) {
        if (defField.type == "boolean" && value === "true") {
          where += " && ".concat(tx, ".").concat(field, "=1");
        } else if (defField.type == "boolean" && value === "false") {
          where += " && ".concat(tx, ".").concat(field, "=0");
