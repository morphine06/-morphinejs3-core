...
class Controller {
  ...
  findone(req, res) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var row = yield _this3._findone(req);
  ...
  create(req, res) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var row = yield _this4._create(req);
  ...
  update(req, res) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var row = yield _this5._update(req);
      if (!row) return res.sendData("not_found");
      res.sendData({
        data: row
      });
  ...
  destroy(req, res) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var oldrow = yield _this6._destroy(req);
      res.sendData({
        data: oldrow
      });
    })();
  ...
  createEmpty(req) {
    ...
    return _asyncToGenerator(function* () {
      var row = _this9.model.createEmpty();

      row[_this9.model.primary] = "";
      return row;
    })();
  }
  ...
  _findone(req) {
    ...
    return _asyncToGenerator(function* () {
      var morePopulate = _arguments2.length > 1 && _arguments2[1] !== undefined ? _arguments2[1] : [];
      var where = "",
          whereData = [],
          row,
          id = req.params.id || req.params[_this10.model.primary];

      if (id * 1 < 0) {
        row = yield _this10.createEmpty(req);
      } else {
        where += "t1.".concat(_this10.model.primary, "=?");
        whereData.push(id);

        var toexec = _this10.model.findone(where, whereData); // Object.entries(this.model.def.attributes).forEach(([field, defField], index) => {
  ...
  _create(req) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var newrow = yield _this11.model.create(req.body).exec(true);
      if (!newrow) return null;
      req.params.id = newrow[_this11.model.primary]; // if (this.modellogevents) await this._log(req, "create", null, newrow);
  ...
  _update(req) {
    ...
    return _asyncToGenerator(function* () {
      var id = req.params.id || req.params[_this12.model.primary],
          where = "".concat(_this12.model.primary, "=?"),
          whereData = [id],
          oldrow;

      if (_this12.modellogevents) {
        oldrow = yield _this12.model.findone(where, whereData).exec();
        if (!oldrow) return null;
      }

      delete req.body[_this12.model.primary];

      _this12._checkPopulateSended(req);

      var row = yield _this12.model.update(where, whereData, req.body).exec();
      if (!row) return null; // if (row.length) newrow = row[0];
      // if (this.modellogevents) await this._log(req, "update", oldrow, newrow);

      return yield _this12._findone(req);
    })();
  }
  ...
  _destroy(req) {
    ...
    return _asyncToGenerator(function* () {
      var updateDeleteField = _arguments3.length > 1 && _arguments3[1] !== undefined ? _arguments3[1] : false;
      var where = "",
          whereData = [],
          oldrow,
          id = req.params.id || req.params[_this13.model.primary];
      where = "".concat(_this13.model.primary, "=?");
      whereData = id;
      oldrow = yield _this13.model.findone(where, whereData).exec();
      if (!oldrow) return null; // if (this.modellogevents) await this._log(req, "destroy", oldrow, null);

      if (updateDeleteField === false) {
        yield _this13.model.destroy(where, whereData).exec();
      } else {
        var d = {};
        d[updateDeleteField] = true;
        yield _this13.model.update(where, whereData, d).exec();
      }
...
function _loadControllers() {
  _loadControllers = _asyncToGenerator(function* () {
    var controllerFiles = _globule.default.find("".concat(process.cwd(), "/src/**/*.controller.js"));
