...
class Controller {
    ...
    this._routes.forEach(route => {
      _App.App[route.method](route.url, (req, res, next) => {
        ...
        req.on("end", function () {
          ...
          console.log("".concat((0, _dayjs.default)().format("YYYY-MM-DD HH:mm:ss"), " - GET ").concat(route.url, " - ").concat(diff));
        });
        next();
      });
  ...
  findExec() {
    ...
    return _asyncToGenerator(function* () {
      ...
      var {
        where,
        whereData
      } = yield _this7.findCreateWhere(req);
      ...
      var {
        limit,
        skip
      } = yield _this7.findCreateLimit(req);
      ...
      if (_this7.debugQuery) console.log("where + orderby + limitreq, whereData", where + orderby + limitreq, whereData);

      var toexec = _this7.model.find(where + orderby + limitreq, whereData);
  ...
  _findone(req) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (id * 1 < 0) {
        ...
        Object.entries(_this9.model.def.attributes).forEach((_ref4, index) => {
  ...
  _compareData(oldData, newData) {
    var compare = {};
    Object.entries(oldData).forEach((_ref5, index) => {
      var [keyoldval, oldval] = _ref5;
  ...
  _create(req) {
    ...
    return _asyncToGenerator(function* () {
      ...
      req.params.id = newrow[_this10.model.primary];
  ...
  _checkPopulateSended(req) {
    Object.entries(this.model.def.attributes).forEach((_ref6, index) => {
      var [field, defField] = _ref6;
  ...
  _update(req) {
    ...
    return _asyncToGenerator(function* () {
      var id = req.params.id || req.params[_this11.model.primary],
  ...
  _destroy(req) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var where = "",
          whereData = [],
          oldrow,
          id = req.params.id || req.params[_this12.model.primary];
      where = "".concat(_this12.model.primary, "=?");
      whereData = id;
      oldrow = yield _this12.model.findone(where, whereData).exec();
      if (!oldrow) return null;
      if (_this12.modellogevents) yield _this12._log(req, "destroy", oldrow, null);
  ...
  _log(req, modelEvent, oldrow, newrow) {
    ...
    return _asyncToGenerator(function* () {
      var id = req.params.id || req.params[_this13.model.primary];
      var c = "";
...
function _loadControllers() {
  _loadControllers = _asyncToGenerator(function* () {
    ...
    for (var i = 0; i < controllerFiles.length; i++) {
      ...
      Object.entries(obj).forEach((_ref7, index) => {
        var [name, constructorFn] = _ref7;
