...
var DbMysql = new class {
  ...
  init(config) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var files = globule.find(process.cwd() + "/src/**/*.model.js"); // console.log("files", files);
      ...
      if (toLink.length) {
        ...
        for (var iLink = 0; iLink < toLink.length; iLink++) {
          ...
          for (var iActualConstraint = 0; iActualConstraint < actualConstraints.length; iActualConstraint++) {
            ...
            var actualConstraintBis = (yield _this2.connection.query(q2))[0]; // console.log("actualConstraint, actualConstraintsBis", actualConstraint, actualConstraintBis);
  ...
  updateTable(def) {
    ...
    return _asyncToGenerator(function* () {
      var describe = yield _this4.connection.query("DESCRIBE " + def.tableName + ""); // console.log("describe", describe);
      ...
      for (var [fieldName, field] of Object.entries(def.attributes)) {
        ...
        if (field.model) {
          ...
          if (_f) {
            type1 = _this4._ormTypeToDatabaseType(_f[0], _f[1]);
            field.type = _f[0];
            field.length = _f[1];
          }
        } else {
  ...
  synchronize(def) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (rows1 === null || _this5.config.migrate == "recreate") exists = false; // console.log("this.config.migrate", this.config.migrate);
