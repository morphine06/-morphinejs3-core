...
module.exports = class DbTableExec {
  ...
  exec() {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!_this2.returnCompleteRow) returnCompleteRow = false; // console.log("this.command,this.data",this.command,this.data);
      ...
      switch (_this2.command) {
        ...
        case "INSERT":
          ...
          query = _this2._createInsertQuery(); // console.log("query", query);
      ...
      try {
        rows = yield _this2.connection.query(query, _this2.whereData, _this2.catchErr); // console.log("rows", rows);
      } catch (error) {
        throw error;
      }
  ...
  postTreatmentMain(rows, returnCompleteRow) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (!rows) {
        ...
        switch (_this3.command) {
          ...
          case "INSERT":
            ...
            // res = this.data ;
            res = null; // if (returnCompleteRow) res = null;
      ...
      if (_this3.def.debug) console.warn("res", res); // console.log('The solution is: ', rows);
      ...
      if (returnCompleteRow && (_this3.command == "UPDATE" || _this3.command == "INSERT")) {
        ...
        if (_this3.command == "UPDATE") {
          ...
          var _rows = yield _this3.table.findone(ftemp).exec(); // if (errsql) console.warn("errsql", errsql);
